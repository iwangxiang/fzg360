<template>
	<view class="oldHouseDetails">
		<view class="swiper">
			<swiper class="iSwiperContent" indicator-dots="true" autoplay="true" interval="5000" duration="500" v-if="topImgs.length">
		...
		<view class="introduction">
			<view>
				<text>售价</text>
				<text>{{houseData.SALE_TOTAL_PRICE > 0 ? houseData.SALE_TOTAL_PRICE + '万' : '总价待定'}}</text>
			</view>
			<view>
				<text>户型</text>
			...
			<view>
				<text>面积</text>
				<text>{{houseData.SALE_AREA > 0 ? houseData.SALE_AREA + '㎡' : '待定'}}</text>
			</view>
		</view>
		<view class="list">
			<view>
				<view>
					<view>
						<text>单价：</text>
						<text>{{houseData.SALE_UNIT_PRICE > 0 ? houseData.SALE_UNIT_PRICE + '元/㎡': '待定'}}</text>
					</view>
					<view @tap="goCalculator">
						房贷计算器
					</view>
				</view>
				<view>
					<view>
						<text>装修：</text>
						<text>{{houseData.SALE_FITMENT ? houseData.SALE_FITMENT : '待定'}}</text>
					</view>
					<view>
						<text>建筑面积：</text>
						<text>{{houseData.SALE_AREA > 0 ? houseData.SALE_AREA + '㎡' : '待定'}}</text>
				...
				<view>
					<view>
						<text>年份：</text>
						<text>{{houseData.SALE_YEAR ? houseData.SALE_YEAR + '年' : '待定'}}</text>
					</view>
					<view>
						<text>楼层：</text>
						<text>{{houseData.SALE_FLOOR}}层/{{houseData.SALE_FLOORS}}层</text>
					</view>
				</view>
				<view>
					<view>
						<text>楼型：</text>
						<text>{{houseData.SALE_TYPE ? houseData.SALE_TYPE : '待定'}}</text>
					</view>
					<view>
						<text>朝向：</text>
						<text>{{houseData.SALE_DIRECT != 0 ? houseData.SALE_DIRECT : '待定'}}</text>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="houseName">
			...
			<view>
				<view>
					<view>
						<text>物业类型：</text>
						<text>{{houseData.SALE_USEAGE ? houseData.SALE_USEAGE : '待定'}}</text>
					</view>
					<view>
						<text>建筑类型：</text>
						<text>{{houseData.SALE_TYPE && houseData.SALE_TYPE != 0 ? houseData.SALE_TYPE : '待定'}}</text>
					</view>
				</view>
				<view>
					<view>
						<text>产权：</text>
						<text>{{houseData.SALE_RIGHT && houseData.SALE_RIGHT != 0 ? houseData.SALE_RIGHT + '年' : '待定'}}</text>
					</view>
					<view>
						<text>物业费：</text>
						<text>{{houseData.SALE_PROPERTY != 0 ? houseData.SALE_PROPERTY + '元/平' : '待定'}}</text>
					</view>
				</view>
				<!-- <view>
					<view>
				...
				<view>
					<view>
						<text>竣工时间：</text>
						<text>{{houseData.SALE_LEAVE_TIME ? houseData.SALE_LEAVE_TIME : '待定'}}</text>
					</view>
				</view>
			</view>
		...
		<view class="related" v-if="relates.length">
			<view><text>相关房源</text></view>
		...
		<view class="footer">
			...
			<view @tap="$util.doCall(user.USER_NAME,user.USER_MOBILE)">
				<fzg-icon class="fuwurexian" type="fuwurexian" size="20" color="#ffffff"></fzg-icon>
				<text>联系经纪人</text>
			</view>
		</view>
	...
	export default {
		onLoad(e) {
			...
			if (this.hasLogin && this.userInfo.mtel) {
				...
				if (uni.getStorageSync('collect_esfids')) { //因为二手房在同一个库里，id唯一，所以不需要区分城市
					this.collectHouseIds = uni.getStorageSync('collect_esfids');
			...
			uni.$once('login', () => { //监听登录事件
				if (this.hasLogin && this.userInfo.hash) {
					if (uni.getStorageSync('collect_esfids')) {
						this.collectHouseIds = uni.getStorageSync('collect_esfids');
						if (this.collectHouseIds.includes(this.id)) {
							this.loveFlag = true;
							this.loveFlagText = '已收藏';
		...
		methods: {
			...
			goLove() {
				this.loveFlagTrue = !this.loveFlagTrue;
				this.loveFlagText = this.loveFlagTrue ? "收藏" : "已收藏"
			},
			closePoP() {
				this.show = false;
			},
			...
			goPath(name) {
				uni.navigateTo({
					url: "/pages/sale/xiaoqu?keyword=" + name
				})
			},
			goCalculator() {
				uni.navigateTo({
					url: "/pages/calcu/loan"
				})
			},
			goCollection(){
				uni.navigateTo({
			...
			doCollect() {
				if(!this.hasLogin || !this.userInfo.hash){
					this.$util.ckLogin('/pages/sale/detail?id='+this.id,'doCollect');
					return;
				}
				let params = {
					aid: this.id,
					...
					if (res.data.status) {
						if (this.loveFlag) {
							if (this.collectHouseIds.includes(this.id)) {
								...
								uni.setStorageSync('collect_esfids', this.collectHouseIds);
							}
							uni.showToast({
								title: res.data.info,
								icon: 'none'
			...
			getData() {
				...
				this.$http.get(this.$api.fuberApi('saleDetail'), {
					data: params
				}).then(res => {
					this.shareParams = {
						title:res.data.info.SALE_SUBJECT,
					...
					broswerRecords[this.$city+res.data.info.ID+'_sale'] = { //以城市+楼盘id+_类型
						id:res.data.info.ID,
						sale_id:res.data.info.SALE_ID,
						sale_subject:res.data.info.SALE_SUBJECT,
						thumb:res.data.info.THUMB_URL ? res.data.info.THUMB_URL : 'https://static.fzg360.com/images/app/esf_nopic.jpg',
						sale_room:res.data.info.SALE_ROOM != 0 ? res.data.info.SALE_ROOM+'室' : '居室暂无',
						sale_area:res.data.info.SALE_AREA != 0 ? res.data.info.SALE_AREA+'㎡' : '面积待定',
						sale_direct:res.data.info.SALE_DIRECT != 0 ? res.data.info.SALE_DIRECT : '朝向暂无',
						sale_total_price:res.data.info.SALE_TOTAL_PRICE > 0 ? res.data.info.SALE_TOTAL_PRICE + '万/套' : '总价待定',
						sale_unit_price:res.data.info.SALE_UNIT_PRICE > 0 ? res.data.info.SALE_UNIT_PRICE + '元/㎡' : '单价待定',
		...
		onShareAppMessage(res) {
			...
			return {
				...
				path: '/pages/sale/detail?id='+this.id,
