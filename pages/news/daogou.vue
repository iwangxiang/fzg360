<template>
	<view class="newHouse">
		...
		<view v-if="listData.length">
			<news-lists v-for="(item, index) in listData" :key="index" :item="item" type="pcnews" @click="goPath"></news-lists>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getData(){
				let params = {catid:1012,page:this.page};
				if(this.keyword) params['keyword'] = this.keyword;
				this.listData.length && (this.status = 'loading');
				this.$http.get(this.$api.siteApi(this.$city,'newsPcList'), {data: params}).then(res => {
					if(this.page == 1){
						uni.showToast({
							title: '请求成功',
							icon: 'success',
