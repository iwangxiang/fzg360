...
<script>
	...
	export default {
		...
		onLoad(){
			if(!uni.getStorageSync('code') || !uni.getStorageSync('tel')){
				uni.redirectTo({
					url: './getpw'
				});
				return;
		...
		computed: {
			hidePhone() {
				let tel = util.hideTel(uni.getStorageSync('tel'));
				return tel;
			},
			verifyMsg(){
				if(this.second == 0){
		...
		methods: {
			...
			getSmsCode(){
				...
				this.second = 60;
				this.isActive = 0;
				this.$http.post(this.$api.userApi('pwGetCode'), {data: {tel:this.tel}}).then(res => {
					if(res.data.status){
			...
			doSubmit() {
				...
				let params = {tel:this.tel,pwd:this.pwd,code:this.code,authCode:uni.getStorageSync('code')};
				this.$http.post(this.$api.userApi('changePwd'), {data: params}).then(res => {
					if(res.data.status){
