<template>
	<view class="pswCon">
		...
		<view class="pswBox">
			...
			<button type="primary" class="pwsButtom" :class="{active: pwsButtom}" hover-class="" @tap="doSubmit">下一步</button><!--输入电话时增加样式active-->
		</view>
...
<script>
    ...
    methods: {
		...
		doSubmit() {
			...
			this.$http.post(this.$api.userApi('checkCode'), {data: params}).then(res => {
				if(res.data.status){
