	...
	<view class="uni-indexed">
		<scroll-view :scroll-into-view="scrollViewId" :style="{ height: winHeight + 'px' }" class="uni-indexed__list" scroll-y>
			<block v-for="(list, idx) in lists" :key="idx">
				<view v-if="list.items && list.items.length > 0" :id="'uni-indexed-list-' + list.index" class="uni-indexed__list-title">{{ list.key }}</view>
				<view v-if="list.items && list.items.length > 0" class="uni-list">
					<view v-for="(item, index) in list.items" :key="item.site" class="uni-list-item" hover-class="uni-list-item--hover">
...
<script>
	...
	export default {
		...
		mounted(){
			let view = uni.createSelectorQuery().in(this).select(".uni-indexed");
		...
		methods: {
			getData(){
				this.$http.get(this.$api.siteApi(this.$city,'cityList')).then(res => {
					let list = res.data.data.citys;
					let winHeight = uni.getSystemInfoSync().windowHeight - this.topHeight;
					this.itemHeight = winHeight / list.length;
			...
			touchStart(e) {
				this.touchmove = true;
				let pageY = e.touches[0].pageY - this.topHeight;
				let index = Math.floor(pageY / this.itemHeight);
				let item = this.lists[index];
				if (item) {
					this.scrollViewId = 'uni-indexed-list-' + item.index;
